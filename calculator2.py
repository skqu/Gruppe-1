...
class data(): 
    def __init__(self) -> None:
        self.history = []
        self.jsonPath = 'history.json'
        self.csvPath = 'history.csv'
    ...
    def read(self):
        with open('history.csv', 'r') as csv_file:
            ...
            for line in csv_reader:
                if len(line) > 0:
                    data_list.append(str(line[0]))      

            return data_list

    def write(self, equation):
        with open('history.csv', 'a') as csv_file:
            fieldnames = ['equation']
...
class Gui(Calculator):
    ...
    def displaySavedHistory(self):
    
        
        for equation in self.read():
